...
class PolicyIterationAgent(Agent):
  ...
  def __init__(self, mdp, discount = 0.9, iterations = 100):
    ...
    self.V = {s: 0 for s in states}
    self.policy = {s: np.random.choice(self.mdp.getPossibleActions(s)) for s in states if len(self.mdp.getPossibleActions(s)) > 0}   # initialise randomly
  ...
  def getQValue(self, state, action):
    ...
    """
    # get all successor states and probabilities and evaluate value of these states
    return self.mdp.getReward(state, action, None) + self.discount * np.sum([self.V[sp] * prob for sp, prob in self.mdp.getTransitionStatesAndProbs(state, action)])
